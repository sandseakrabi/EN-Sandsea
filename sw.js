...
const GAS_URL   = 'https://script.google.com/macros/s/AKfycbwViAK68tokk2jPOEx5nkSyVuCIVLHVzMfsCRXM12Ea9I3fl58VOTgNWLiFFlFWE2827Q/exec';
...
async function checkTicketsInBackground() {
  ...
  try {
    ...
    try { await dbSet('last_sw_error', null); } catch(_) {}
  } catch (e) {
    // เงียบไว้ — รอบหน้าค่อยลองใหม่ ไม่ต้อง throw ให้ระบบ retry รัว ๆ
    // แต่บันทึก error ล่าสุดไว้ใน IndexedDB เผื่อต้อง debug ทีหลัง (เช่น GAS_URL ไม่ตรงกับ index.html)
    console.warn('[SW] checkTicketsInBackground error:', e);
    try { await dbSet('last_sw_error', { message: String(e && e.message || e), time: new Date().toISOString(), url: GAS_URL }); } catch(_) {}
  }
}
